...
function ArsipDosen() {
  ...
  const handleShow = (isAdding, id = null) => {
    ...
    if (!isAdding && id) {
      const arsipDsnToEdit = platforms.find(
        (platform) => platform.idArsip === id
      );
      if (arsipDsnToEdit) {
        setNewRecord({
          ...newRecord,
          idArsip: arsipDsnToEdit.idArsip,
          idDosen: arsipDsnToEdit.idDosen,
          namaFile: arsipDsnToEdit.namaFile,
          kategori: arsipDsnToEdit.kategori,
          jenis: arsipDsnToEdit.jenis,
          tglUpload: arsipDsnToEdit.tglUpload,
        });
      }
    } else {
  ...
  const getPlatforms = async () => {
    try {
      const response = await axios.get("http://localhost:5000/arsip-dsn");
  ...
  useEffect(() => {
    ...
    if (editRecordId !== null) {
      axios
        .get(`http://localhost:5000/arsip-dsn/${editRecordId}`)
        .then((response) => {
  ...
  const handleAddPlatform = () => {
    ...
    axios
      .post("http://localhost:5000/arsip-dsn", formData)
      .then((response) => {
  ...
  const handleEditPlatform = () => {
    ...
    axios
      .patch(`http://localhost:5000/arsip-dsn/${editRecordId}`, formData)
      .then((response) => {
  ...
  const handleDeletePlatform = (idArsip) => {
    axios
      .delete(`http://localhost:5000/arsip-dsn/${idArsip}`)
      .then((response) => {
                ...
                <form>
                  ...
                  <div className="form-group mt-3">
                    ...
                    >
                      ...
                      {dosen.map((dsn) => (
                        <option key={dsn.idDosen} value={dsn.idDosen}>
                          {dsn.nama}
                        </option>
                      ))}
                    </select>
